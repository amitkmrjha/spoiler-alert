...
object SpoileralertBehavior {
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[SpoileralertCommand, SpoileralertEvent, SpoileralertState](
        persistenceId = persistenceId,
        emptyState = SpoileralertState.initial,
...
/**
  ...
  */
case class SpoileralertState(message: String, timestamp: String) {
  def applyCommand(cmd: SpoileralertCommand): ReplyEffect[SpoileralertEvent, SpoileralertState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: SpoileralertEvent): SpoileralertState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[SpoileralertEvent, SpoileralertState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[SpoileralertEvent, SpoileralertState] =
    Effect
...
object SpoileralertState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: SpoileralertState = SpoileralertState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[SpoileralertCommand]("SpoileralertAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[SpoileralertState] = Json.format
}
...
/**
  ...
  */
case class GreetingMessageChanged(message: String) extends SpoileralertEvent
...
/**
  ...
  */
sealed trait SpoileralertCommand
    extends SpoileralertCommandSerializable
...
object SpoileralertSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    ...
    JsonSerializer[SpoileralertState],
